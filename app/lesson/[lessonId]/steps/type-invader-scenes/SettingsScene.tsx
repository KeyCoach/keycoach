...
export class SettingsScene extends Scene {
  ...
  create() {
    ...
    const themeButtons = themes.map((theme, index) => {
      ...
      navigationItems.push({
        ...
        onSelect: () => this.setTheme(theme.toLowerCase() as GameSettings["theme"]),
      });
    ...
    const soundText = this.add
      .text(soundToggle.x, soundToggle.y, gameSettings.soundEnabled ? "ON" : "OFF", {
        fontSize: "20px",
        fontFamily: "Monospace",
  ...
  private setTheme(theme: GameSettings["theme"]) {
    ...
    themeButtonsTexts.forEach((text) => {
      text.setColor(gameSettings.theme === text.text.toLowerCase() ? colors.yellow : colors.white);
    });
  ...
  private toggleSound(soundText: Phaser.GameObjects.Text) {
    ...
    console.log(`Sound toggled to ${gameSettings.soundEnabled ? "enabled" : "disabled"}`);
