...
export function FingerPlacementAnalysis({ attempt }: { attempt: Attempt }) {
  ...
  const getFingerMistakesAnalysis = () => {
    ...
    const fingerMistakes = attempt.mistakes.filter(
      (mistake) => mistake.status === Letter.WrongFinger
    );

    if (fingerMistakes.length === 0) {
      return (
        <div className="p-4 bg-green-100 rounded-lg dark:bg-green-900 dark:text-green-200">
    ...
    if (attempt.keyStrokes && attempt.keyStrokes.length > 0) {
      ...
      // Sort by frequency
      const sortedErrors = Object.entries(fingerErrorMap)
      ...
      const sortedKeyMistakes = Object.entries(keyMistakesMap)
        .sort((a, b) => b[1] - a[1]);

      return (
        <div className="space-y-6">
          {/* Problem keys summary */}
          <div className="p-4 bg-amber-100 rounded-lg dark:bg-amber-900">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {sortedKeyMistakes.map(([key, count]) => (
                <div 
                  key={key} 
                  className="flex items-center justify-between p-3 bg-white dark:bg-slate-700 rounded-md shadow-sm"
                >
                  <div className="flex items-center justify-center w-10 h-10 bg-amber-200 dark:bg-amber-800 rounded-lg font-bold text-amber-800 dark:text-amber-200">
                    {key}
                  </div>
                  <div className="text-right">
                    <div className="text-slate-500 dark:text-slate-400 text-xs">mistakes</div>
          ...
          <div className="space-y-4">
            ...
            {sortedErrors.map(([fingerPair, data], index) => {
              ...
              const sortedKeys = Object.entries(data.keys)
                .sort((a, b) => b[1] - a[1]);
              
              return (
                <div key={index} className="bg-white dark:bg-slate-700 rounded-lg shadow p-4">
                  <div className="flex justify-between items-center mb-3">
                    <div className="font-medium text-slate-800 dark:text-slate-200">
                      Used {formatFingerName(usedFinger)} instead of {formatFingerName(correctFinger)}
                    </div>
                    <div className="text-cerulean-600 dark:text-cerulean-400 font-bold">
                      {data.count} times
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {sortedKeys.map(([key, keyCount]) => (
                      <div key={key} className="flex items-center space-x-1 bg-slate-100 dark:bg-slate-600 px-3 py-1 rounded-full">
                        <span className="font-mono font-bold text-slate-700 dark:text-slate-300">{key}</span>
                        <span className="text-xs text-slate-500 dark:text-slate-400">×{keyCount}</span>
                      </div>
                    ))}
                  </div>
                </div>
    ...
    const sortedKeyMistakes = Object.entries(keyMistakesMap)
      ...
      return (
        <div className="space-y-4">
          <div className="bg-amber-50/70 dark:bg-amber-900/50 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
            <p className="text-amber-700 dark:text-amber-300 text-sm flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-amber-500" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
              </svg>
              {fingerMistakes.length} finger placement errors detected. Focus on using correct fingers for each key.
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
            {sortedKeyMistakes.slice(0, 4).map(([key, count]) => (
              <div 
                key={key} 
                className="bg-white dark:bg-slate-700 rounded-lg shadow-sm p-3 text-center"
              >
