...
import { fetchWords } from "@/constants/wordsApi";
...
export class GameMechanics {
  ...
  recordTypingError(char: string) {
    ...
    this.typingStats.characterErrors.set(char, this.typingStats.characterErrors.get(char)! + 1);
  }
  ...
  recordWordAttempt(word: string, errorCount: number) {
    ...
    if (errorCount > 0) {
      ...
      this.typingStats.wordErrors.set(word, this.typingStats.wordErrors.get(word)! + 1);
    }
  }
  ...
  getTypingStats() {
    ...
    const accuracyValue =
      this.typingStats.totalKeysPressed > 0
        ? (this.typingStats.correctKeysPressed / this.typingStats.totalKeysPressed) * 100
        : 100;
  ...
  spawnAsteroid() {
    ...
    do {
      ...
      safeSpawn = this.asteroids.every(
        (asteroid) => Phaser.Math.Distance.Between(x, y, asteroid.sprite.x, asteroid.sprite.y) > 80,
      );
      attempts++;
  ...
  private getAsteroidSpawnX(): number {
    ...
    this.asteroids.forEach((asteroid) => {
      xPositions = xPositions.filter((x) => Math.abs(x - asteroid.sprite.x) > minDistance);
    });
  ...
  private createAsteroidSprite(x: number, wordWidth: number): Phaser.GameObjects.Sprite {
    const sprite = this.scene.add.sprite(x, -50, themeManager.getAsset("asteroid"));
  ...
  destroyAsteroid(index: number) {
    ...
    const particles = this.scene.add.particles(asteroid.sprite.x, asteroid.sprite.y, "particle", {
    ...
    for (let i = this.asteroids.length - 1; i >= 0; i--) {
      ...
      const effectiveY =
        themeManager.getCurrentTheme() === "birthday"
          ...
          : asteroid.sprite.y + (asteroid.sprite.height * asteroid.sprite.scaleY) / 2; // Bottom of sprite
